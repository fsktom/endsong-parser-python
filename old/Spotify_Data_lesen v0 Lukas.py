...
class Gain_Data:
                ...
                if f["spotify_track_uri"]:
                    ...
                    while i < len(self.__info) and not match:
                        if self.__info[i]["ID"] == f["spotify_track_uri"]:
                            self.__info[i][
                                "AnzahlStreams"
                            ] += 1  # am ende durch länge der zeitpunkte?
                        ...
                        self.__info += [
                            {
                                ...
                                "Titel": f["master_metadata_track_name"],
                                "Künstler": f["master_metadata_album_artist_name"],
                                "AnzahlStreams": 1,
        ...
        for e in lpath:
            ...
            for f in file:
                if (
                    ...
                    while i < len(self.__info) and not match:
                        if (
                            self.__info[i]["Titel"] == f["master_metadata_track_name"]
                            and self.__info[i]["Künstler"]
    ...
    def set_bonds(self):
        ...
        """
        self.__leftbond = self.__convert_to_unix(
            input("Linke Grenze (yyyy.mm.dd-hh.mm.ss): "), 2
        )
        self.__rightbond = self.__convert_to_unix(
            input("Rechte Grenze (yyyy.mm.dd-hh.mm.ss): "), 2
        )
...
class output_Data:
    ...
    def print_console(
        self, array, Titel=True, Künstler=True, Album=False, Streams=True
    ):
        for i in range(len(topsongs)):
            string = str(i + 1)
            if Titel:
                string += ". Titel: " + topsongs[i]["Titel"]
            if Künstler:
                string += ", Künstler: " + topsongs[i]["Künstler"]
            if Album:
                string += ", Album: " + topsongs[i]["Album"]
            if Streams:
                string += ", Streams: " + str(topsongs[i]["AnzahlStreams"])
            print(string)
