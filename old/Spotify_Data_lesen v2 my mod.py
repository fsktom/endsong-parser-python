                ...
                for _ in range(len(e["album"])-1):
                    for i in range(len(e["album"])-1):
                        if e["album"][i][1] < e["album"][i+1][1]:
                            e["album"][i], e["album"][i+1] = e["album"][i+1], e["album"][i]
...
class output_Data():
    ...
    def print_top_songs(self,aspect,title=True,artist=True,album=False,streams=True,mainindex=10,secindex=5,percentages=False):
                ...
                if title:
                    ...
                    try:
                        for j in range(1,secindex):
                            ...
                            if streams: string += ", Streams: " + str(array[i]["title"][j][1])
                    except IndexError:
                        pass
                    string += "}"
                if album:
                    string += ", Most Played Albums: {1. " + array[i]["album"][0][0]
                    if streams: string += ", Streams: " + str(array[i]["album"][0][1])
                    try:
                        for j in range(1,secindex):
                            string += "; " + str(j+1) + ". " + array[i]["album"][j][0]
                            if streams: string += ", Streams: " + str(array[i]["album"][j][1])
                    except IndexError:
                        pass
                    string += "}"
                print(string)
    ...
    def print_first_last(self,fl=True):
        ...
        """
        if fl: song = self.first
        elif not fl: song = self.last
        print("Song " + song[0]["title"] + " from " + song[0]["artist"] + " from " + song[0]["album"] + ", Streams: " + str(song[0]["streams"]) + ", first/last listened at " + str(dt.datetime.utcfromtimestamp(song[1]) + dt.timedelta(hours=+1)))

    def print_aspect(self,aspect,name,title=True,artist=True,album=False,streams=True,index=5,percentages=False):
                    ...
                    if title:
                        ...
                        try:
                            for j in range(1,index):
                                ...
                                if streams: string += ", Streams: " + str(e["title"][j][1])
                        except IndexError:
                            pass
                        string += "}"
                    if album:
                        string += ", Most Played Albums: {1. " + e["album"][0][0]
                        if streams: string += ", Streams: " + str(e["album"][0][1])
                        try:
                            for j in range(1,index):
                                string += "; " + str(j+1) + ". " + e["album"][j][0]
                                if streams: string += ", Streams: " + str(e["album"][j][1])
                        except IndexError:
                            pass
                        string += "}"
                    print(string)
